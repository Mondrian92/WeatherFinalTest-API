...
router.post(
  ...
  validationHandler,
  async ({ body: { email, username, password } }: Request, res: Response) => {
    if (
...
router.delete(
  ...
  validationHandler,
  async ({ headers: { email, token } }: Request, res: Response) => {
    if (
      (await client.existsAsync(token)) &&
      (await client.getAsync(token)) === email
    ) {
      client.del(token);
      ...
      res
        .status(400)
        .json({ message: "utente non loggato, impossibile eseguire logout" });
    }
  }
);
